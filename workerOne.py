...
from env_variables import QPX_API_KEY, ORIGIN_ONE, DESTINATION_ONE, DEPARTURE_DATE
...
params = myCluster.getParams(ORIGIN_ONE, DESTINATION_ONE, DEPARTURE_DATE, 100)
...
while True:
    if last != datetime.now().hour:
        ...
        concat = time + ", " + price + ", " + carrier + ", " + ORIGIN_ONE + ", " + DESTINATION_ONE + ", " + DEPARTURE_DATE
        ...
        myCluster.insertData('Tickets', ['time', 'day', 'price', 'carrier', 'origin', 'destination', 'departure_date', 'concat'], 
            [time, str(day_of_week), price, carrier, ORIGIN_ONE, DESTINATION_ONE, DEPARTURE_DATE, concat])
